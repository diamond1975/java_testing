  ...
  private void save(List<ContactData> contacts, File file) throws IOException {
    ...
    for (ContactData contact : contacts) {
      writer.write(String.format("%s;%s;%s;%s;%s;%s;%s;%s;%s\n", contact.getName1(), contact.getName2(), contact.getName3(),
              contact.getAddress(), contact.getEmail1(),contact.getEmail2(),contact.getEmail3(), contact.getMobile(), contact.getGroup()));
    }
    writer.close();
  }
  ...
  private List<ContactData> generateContact(int count) {
    ...
    for (int i = 0; i < count; i++) {
      contact.add(new ContactData().withName1(String.format("Gabdullin %s", i)).withName2 (String.format("Almaz %s", i))
              ...
              .withMobile(String.format("252685", i)).withGroup(String.format("diamind", i)));
    }
    return contact;
  }
}
